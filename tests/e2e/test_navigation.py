...
class TestLinkedInNavigationE2E:
    ...
    async def test_full_extraction(self, browser_manager, navigator, tmp_path):
        ...
        try:
            ...
            await navigator.navigate_to_posts(page, test_profile)
            post_elements = await navigator.load_posts(page, min_posts=3, max_seconds=20)
            ...
            if post_elements:
                ...
                for post in posts_data:
                    ...
                    required_fields = ['author_name', 'posted_at', 'text', 'hashtags', 'links', 'reactions_count', 'comments_count']
                    for field in required_fields:
                        assert field in post
            
        finally:
            await browser_manager.close()
                feed_elements = await test_page.query_selector_all('div.feed-shared-update-v2, div.feed-shared-update')
                profile_elements = await test_page.query_selector_all('div.profile-topcard')
                
                if feed_elements or profile_elements:
                    await test_page.close()
                    self.logger.info("Restored session from storage state")
                else:
                    await self.context.close()
                    self.context = None
                    self.logger.warning("Storage state invalid, starting fresh session")
            except Exception as e:
                self.logger.error(f"Error loading storage state: {e}")
                if self.context:
                    await self.context.close()
                self.context = None