...
class PostExtractor:
    ...
    async def extract_post_data(self, post_element: ElementHandle, page: Page) -> Optional[Dict]:
    ...
    async def _extract_post_id(self, post_element: ElementHandle) -> Optional[str]:
    ...
    async def _extract_author_name(self, post_element: ElementHandle) -> Optional[str]:
    ...
    async def _extract_timestamp(self, post_element: ElementHandle) -> Optional[str]:
    ...
    async def _extract_text_content(self, post_element: ElementHandle) -> Optional[str]:
    ...
    def _extract_hashtags(self, text: str) -> List[str]:
        ...
        return sorted(list(set(tag.lower() for tag in hashtags if tag)))
    
    async def _extract_links(self, post_element: ElementHandle, base_url: str) -> List[str]:
    ...
    async def _extract_engagement_counts(self, post_element: ElementHandle) -> Tuple[Optional[int], Optional[int]]:
        """Extract reaction and comment counts."""
        reactions_count = None
        comments_count = None
        ...
        try:
            ...
            for selector in engagement_selectors:
                ...
                if element:
                    ...
                    aria_label = await element.get_attribute('aria-label')
                    if aria_label:
                        reactions_count, comments_count = self._parse_engagement_counts(aria_label)
                    ...
                    text = await element.inner_text()
                    if text:
                        reactions_count, comments_count = self._parse_engagement_counts(text)
                        if reactions_count is not None or comments_count is not None:
                            break
            ...
            if reactions_count is None:
                reactions_count = await self._extract_single_count(
                    post_element, 
                    ['[aria-label*="reaction"]', '.reactions-count', '[data-test-id="reactions-count"]']
                )
            
            if comments_count is None:
                comments_count = await self._extract_single_count(
                    post_element,
                    ['[aria-label*="comment"]', '.comments-count', '[data-test-id="comments-count"]']
                )
    ...
    async def _extract_single_count(self, post_element: ElementHandle, selectors: List[str]) -> Optional[int]:
    ...
    def _parse_engagement_counts(self, text: str) -> Tuple[Optional[int], Optional[int]]:
        """Parse engagement counts from text like '120 reactions • 45 comments'."""
        reactions_count = None
        comments_count = None
        
        # Look for patterns like "X reactions" and "Y comments"
        reactions_match = re.search(r'(\d+(?:,\d+)*)\s+(?:reaction|like)', text, re.IGNORECASE)
        if reactions_match:
    ...
    def _parse_single_count(self, text: str) -> Optional[int]:
