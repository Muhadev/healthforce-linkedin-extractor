...
def extract(
    ...
    min_posts: int = typer.Option(
        10, 
        "--min-posts", 
        help="Minimum number of posts to extract"
    ),
    max_seconds: int = typer.Option(
        60, 
        "--max-seconds", 
        help="Maximum extraction time in seconds"
    ),
    ...
    console.print(f"Starting LinkedIn Posts Extractor", style="bold green")
...
async def run_extraction(
    ...
    try:
        ...
        with Progress(
            ...
            # Extract post data
            progress.update(task, description=f"Extracting data from {len(post_elements)} posts...")
