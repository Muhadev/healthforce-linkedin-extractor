...
class LinkedInPost(BaseModel):
    ...
    posted_at: Optional[datetime] = Field(None, description="Post timestamp in UTC")
    ...
    reactions_count: Optional[int] = Field(None, ge=0, description="Number of reactions")
    ...
    def normalize_hashtags(cls, v: Any) -> List[str]:
        ...
        if isinstance(v, str):
            v = [v]
        return sorted(list(set(tag.lower().strip('#') for tag in v if tag)))
...
class LinkedInPostsExtraction(BaseModel):
    ...
    posts: List[LinkedInPost] = Field(default_factory=list, description="Extracted posts")
    
    extraction_duration_seconds: Optional[float] = Field(None, ge=0)
    reason: Optional[str] = Field(None, description="Reason for stopping (timeout, min_met, etc.)")
