...
class LinkedInNavigator:
    ...
    async def load_posts(
        ...
        self.logger.info(
            f"Post loading completed",
            context={
                'total_posts_loaded': len(posts),
                'elapsed_seconds': time.time() - start_time,
                'min_posts_target': min_posts,
                'action_metrics': self.action_tracker.get_metrics()
            }
        )
    ...
    async def extract_posts_data(self, post_elements: List[ElementHandle], page: Page) -> List[dict]:
